...
class SceneActivity : ArActivity<ActivitySceneBinding>(ActivitySceneBinding::inflate) {
    ...
    // COMENTADO: Variável de desenho não é mais necessária - apenas medidor permanece ativo
    // private var drawing: Drawing? = null

    // COMENTADO: Conjunto de views de propriedades materiais não é mais necessário
    ...
    override fun onArResumed() {
        ...
        // COMENTADO: Informações da câmera ocultadas
    ...
    private fun initSceneBottomSheet() = with(bottomSheetScene) {
        ...
        model.selection.observe(this@SceneActivity) {
            body.apply {
                // COMENTADO: Ícones ocultados - apenas medidor permanece visível
            ...
            // COMENTADO: Referência ao desenho oculta - apenas medidor permanece ativo
            // header.add.requestDisallowInterceptTouchEvent = it == Drawing::class
        }

        body.apply {
            // COMENTADO: Listeners dos ícones ocultados
    ...
    private fun initNodeBottomSheet() = with(bottomSheetNode) {
        ...
        body.apply {
            // COMENTADO: Controles de propriedades materiais ocultados
    ...
    private fun initAr() = with(arSceneView) {
        ...
        scene.addOnPeekTouchListener { hitTestResult, motionEvent ->
            ...
            // COMENTADO: Funcionalidade de desenho oculta - apenas medidor permanece ativo
    ...
    private fun shouldHandleDrawing(motionEvent: MotionEvent? = null, hitTestResult: HitTestResult? = null): Boolean {
        ...
        return false // Sempre retorna false pois apenas medidor está ativo
    }
    ...
    private fun createNodeAndAddToScene(anchor: () -> Anchor, focus: Boolean = true) {
        when (model.selection.value) {
            // COMENTADO: Tipos de nós ocultados - apenas medidor permanece ativo
    ...
    private fun onArUpdateBottomSheet(state: TrackingState?) = with(bottomSheetScene) {
        ...
        when (behavior().state) {
            STATE_HIDDEN, STATE_COLLAPSED -> Unit
                ...
                // COMENTADO: Componentes de posição, rotação e cena ocultados
        ...
        when {
            node != coordinator.selectedNode || node != coordinator.focusedNode || behavior().state == STATE_HIDDEN -> Unit
                ...
                with(header) {
                    ...
                    // COMENTADO: Distância da câmera oculta - apenas medições permanecem
                    // distance.text = arSceneView.arFrame?.camera.formatDistance(this@SceneActivity, node)
                    // COMENTADO: Controles de Cloud Anchor ocultados - apenas medidor permanece ativo
                ...
                with(body) {
                    // COMENTADO: Componentes de posição, rotação e escala ocultados
                    ...
                    // COMENTADO: Valores de Cloud Anchor ocultados - apenas medições permanecem
    ...
    private fun onNodeFocused(node: Nodes?) {
        ...
        when (node) {
            ...
            coordinator.selectedNode -> {
                ...
                // COMENTADO: Visibilidade dos controles de propriedades materiais ocultada
                ...
                // COMENTADO: Visibilidade dos controles de Cloud Anchor ocultada - apenas medidor permanece ativo
